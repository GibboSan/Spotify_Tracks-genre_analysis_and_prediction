...
```{r, echo=FALSE}
# Logistic regression with variable selection
roc.lrvs.hiphop <- roc(test$hiphop, logistic_prop_hiphop_under, levels=c(0, 1))
plot(roc.out.hiphop, legacy.axes=TRUE, 
     main= 'ROC curves for Hip-hop models',
     xlab="False positive rate", ylab="True positive rate")

# Logistic regression with ridge regularization
ridge.prob.hiphop <- predict(hiphop.ridge.cv, 
                                      X.reggaeton.test,
                                      s = best.lambda.ridge.hiphop,
                                      type = 'response')

roc.ridge.hiphop <- roc(test$hiphop, ridge.prob.hiphop, levels=c(0, 1))

lines(roc.ridge.hiphop, col = "purple", lty = 1)

# Logistic regression with lasso regularization
lasso.prob.hiphop <- predict(hiphop.lasso.cv, 
                                      X.hiphop.test,
                                      s = best.lambda.lasso.hiphop,
                                      type = "response")

roc.lasso.hiphop <- roc(test$hiphop, lasso.prob.hiphop, levels=c(0, 1))

lines(roc.lasso.hiphop, col = "blue", lty = 1)

# LDA
lda.prob.hiphop <- predict(lda.fit, test, type='response')
lda.prob.hiphop <- lda.prob.hiphop$posterior[,4]

roc.lda.hiphop <- roc(test$hiphop, lda.prob.hiphop, levels=c(0, 1))

lines(roc.lda.hiphop, col = "yellow", lty = 1)

# QDA
qda.prob.hiphop <- predict(qda.fit, test, type='response')
qda.prob.hiphop <- qda.prob.hiphop$posterior[,4]

roc.qda.hiphop <- roc(test$hiphop, qda.prob.hiphop, levels=c(0, 1))

lines(roc.qda.hiphop, col = "orange", lty = 1)

# Naive Bayes
nb.prob.hiphop <- predict(nb.fit, test[,1:16], type='raw')
nb.prob.hiphop <- nb.prob.hiphop[,4]

roc.nb.hiphop <- roc(test$hiphop, nb.prob.hiphop, levels=c(0, 1))

lines(roc.nb.hiphop, col = "red", lty = 1)
...
```
```{r, echo=FALSE}
print('Auc for Hip-hop classification models')
```

--------------------------------------------------

```{r, echo=FALSE}
print(paste('Logistic regression with model selection: AUC =', round(auc(roc.lrvs.hiphop),3)))
print(paste('Ridge logistic regression:                AUC =', round(auc(roc.ridge.hiphop),3)))
print(paste('Lasso logistic regression:                AUC =', round(auc(roc.lasso.hiphop),3)))
print(paste('LDA model:                                AUC =', round(auc(roc.lda.hiphop),3)))
print(paste('QDA model:                                AUC =', round(auc(roc.qda.hiphop),3)))
print(paste('Naive Bayes model:                        AUC =', round(auc(roc.nb.hiphop),3)))
```
